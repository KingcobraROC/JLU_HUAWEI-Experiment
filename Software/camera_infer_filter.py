...
from filter import CommandFilter  #用于滤波
...
def main():
    ...
    command_filter = CommandFilter(window_size=5)  #设置滤波窗口大小
                ...
                for idx, box in enumerate(pred_all):
                    ...
                    if conf > 0.4:  # 置信度阈值
                        ...
                        if class_name.lower() == "pink":  # 适配可能的名称
                            ...
                            # 计算小车移动指令并进行指令滤波
                            raw_movement_command = car_movement(distance_cm, center_x, frame.shape[1])
                            filtered_movement_command = command_filter.update(raw_movement_command)

                            # 发送指令
                            if ser:
                                try:
                                    msg = cmd2msg(filtered_movement_command)
                                    send_once(ser, msg)
                                    logging.info(f"Sent command: {msg} for movement: {filtered_movement_command}")
                                except Exception as e:
                            ...
                            movement_label = f'Move: {filtered_movement_command}'
