...
# 串口通信函数
from control_wifi import ser_init, send_msg, car_movement, cmd2msg
...
def main():
    ...
    try:
        ...
        ser = None
    # 初始化摄像头
    url = "http://admin:123456@10.176.218.182:8081"
    ...
    outfile = "/root/motor_exp/yolo_detect/video_result_2.mp4"
    ...
    try:
        ...
        while cap.isOpened():
            ...
            boxout = nms(
                torch.tensor(output), conf_thres=0.6, iou_thres=0.5
            ...
            if len(boxout) > 0 and boxout[0] is not None:
                ...
                for idx, box in enumerate(pred_all):
                    ...
                    if conf > 0.6:  # 置信度阈值
                        ...
                        # 如果检测到粉色物体，计算距离和移动指令
                        if class_name.lower() == "yellow_cube":
                            # 计算距离（使用物体的宽度进行计算）
